...
Different *Symbiodiniaceae* species and populations can convey differnet thermal tolerances to their coral host. However, the genes that control symbiont thermal tolerance are still unclear. The original data authors attempted to address this gap in knowledge by comparing two heterogenous populations of type C1 from the same coral species living in two distinct sites. The population from South Molle Island (SM) is considered thermally sensitive while the population from Magnetic Island (MI) is thermally tolerant allowing them to compare differentially expressed genes based on thermal tolerance level. We are now using their data but analyzing it through a different pathway to determine what genes are differentially expressed.
